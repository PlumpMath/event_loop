...
#ifndef WIN32
#define ELOOP_WATCHDOG_ADDON 1
#define ELOOP_EXCEPTION_ADDON 1
#define ELOOP_CALLBACKLONGDEBUG_ADDON 1
#else
#define ELOOP_WATCHDOG_ADDON 0
#define ELOOP_EXCEPTION_ADDON 0
#define ELOOP_CALLBACKLONGDEBUG_ADDON 0
#endif
...
void  eloop_timeout_handler_for_WDT(void *eloop_ctx, void *timeout_ctx);
extern int eloop_restartWDT(void *eloop_ctx,int secs,int usecs);
extern int eloop_startWDT(void *eloop_ctx,int secs,int usecs);
extern int eloop_stopWDT(void *eloop_ctx);
extern int eloop_openWDT(void *eloop_ctx);
extern int eloop_closeWDT(void *eloop_ctx);
...
#define ELOOP_MY_SOCK_RET_RECEIVED 2
#define ELOOP_MY_SOCK_RET_TIMEOUT 1
#define ELOOP_MY_SOCK_RET_TERMINATED 0
int eloop_run_with_mySocket(int sock,unsigned int secs, unsigned int usecs);
#if 0
typedef void (*gethostbyname_ext_eloop_res)(int bTimeout,struct hostent *hp,void* userCtx);
void gethostbyname_ext_eloop_reconf();
struct hostent *gethostbyname_ext_eloop_req(char* if_name_in,const char *name_in, int serverchoice_in,int dwTimeout_in,gethostbyname_ext_eloop_res onCompleted,void* userCtx);//self configure from /var/etc/resolv.conf
...
void gethostbyname_ext_free_result(struct _DNSCtx* pdnsCtx);
typedef void (*OnDNScompleteFunc)(struct _DNSCtx* pdnsctx,struct hostent *hp,void* userCtx);
...
int dup_hostentry(struct hostent* pheIn,struct hostent* pheOut,char buf[sizeof(struct in_addr) +
			...
			sizeof(char *)*(8/*ALIAS_DIM*/) + 384/*namebuffer*/ + 32/* margin */],int buflen,int * h_errnop);
